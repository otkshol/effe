...
- 原因2:リスナーやコールバック
    - リスナー
        - リスナーのインスタンス自身がメモリリークの原因なっていそう（例:アプリケーション起動時にイベント実行するリスナーってアプリが立ち上がっている間はメモリリークになる）
    - コールバック（Android実装でよくある事例？）
        - コールバック登録をしても登録を解除しないケース
            - [参考記事](https://qiita.com/KeithYokoma/items/d6c3bd07fc57b93f91cc)
            - [コードサンプル](https://qiita.com/softbase/items/94984fa7821ecbc02f5b)
    - クライアントがコールバックを登録するが明示的に登録を解除しないAPIを実装するとき、コールバックが蓄積される。
    - 解決方法
...
メモリリークもできるだけ早い段階で検知できることが望ましい。`OutOfMemoryError` の調べ方を
[こちら](https://qiita.com/opengl-8080/items/64152ee9965441f7667b#mark--sweep)を参考に追っかけてみた。性能改善タスクとかで使えそう。


---
### 余談
- **長さ(length)とサイズ(size)って違うっけ？**
